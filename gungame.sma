...
gg_afk_protection, gg_stats_split, gg_top10_ppp, canver;
...
public plugin_init()
{
	...
	canver = register_cvar("gg_killer_hp","10");
	// autovote cvars
	...
	gg_warmup_timer_setting = register_cvar("gg_warmup_timer_setting","45");
	gg_warmup_weapon = register_cvar("gg_warmup_weapon",KNIFE);
	gg_warmup_multi = register_cvar("gg_warmup_multi","1");
...
{
	register_native("gg_give_player_bazooka", "native_give_bazooka")
}

public native_give_bazooka
{
	if(!is_user_alive(id)) return 0;

	return Command_GiveWeapon(id);
...
public plugin_precache()
{
	...
	gg_sound_winner = register_cvar("gg_sound_winner","media/bam-bam.wav");
...
public ham_player_killed_pre(victim,killer,gib)
{
	...
	if(killer == victim || !is_user_connected(killer) || cs_get_user_team(killer) == cs_get_user_team(victim))
		return HAM_IGNORED;
	new cancek = get_pcvar_num(canver)
	set_user_health(killer,get_user_health(killer) + cancek)
	set_hudmessage(255, 0, 0, 0.38, 0.14, 0, 6.0, 12.0);
	show_hudmessage(killer ,"Adam Vurdugun Icin Bizden Sana %d  HP",cancek);
...
public show_win_screen(params[66]) // [winner,loser,gg_win_motd[64]]
{
	...
	for(player=1;player<=maxPlayers;player++)
	{
		...
		len += formatex(motd[len],1535-len,"<body bgcolor=black style=line-height:1;color:white><center><img src=^"https://i.hizliresim.com/vEFdvC.png^"><p>");
...
if(equal(weapon, "weapon_bazooka")
return gg_give_player_bazooka(id);
