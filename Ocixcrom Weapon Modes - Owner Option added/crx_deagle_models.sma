...
#define DEFAULT_V "models/v_deagle.mdl"
#define DEFAULT_P "models/p_deagle.mdl"
...
public plugin_init()
{
	register_plugin("Deagle Models", PLUGIN_VERSION, "OciXCrom")
	register_cvar("CRXAWPModels", PLUGIN_VERSION, FCVAR_SERVER|FCVAR_SPONLY|FCVAR_UNLOGGED)
	
	if(!g_iAWPNum)
		set_fail_state("No Deagles found in the configuration file.")
	
	register_dictionary("DeagleModels.txt")
	
	RegisterHam(Ham_Spawn, "player", "OnPlayerSpawn", 1)
	RegisterHam(Ham_Item_Deploy, "weapon_deagle", "OnSelectAWP", 1)
	
	register_clcmd("say /deagle", "ShowMenu")
	register_clcmd("say_team /deagle", "ShowMenu")
...
ReadFile()
{
	...
	formatex(szFilename, charsmax(szFilename), "%s/DGLModels.ini", szConfigsName)
		...
		while(!feof(iFilePointer))
		{
			...
			switch(szData[0])
			{
				...
				default:
				{
					...
					else if(equal(szKey, "OWNER"))
    					eAWP[OWNER] = read_flags(szValue)
...
public ShowMenu(id)
{
	...
	formatex(szTitle, charsmax(szTitle), "%L", id, "DGM_MENU_TITLE")
		...
		if(g_bRankSystem)
		{
			if(eAWP[LEVEL] && iLevel < eAWP[LEVEL])
			{
				if(eAWP[SHOW_RANK])
				{
					...
					format(szItem, charsmax(szItem), "%s %L", szItem, id, "DGM_MENU_RANK", szRank)
				}
				else
					format(szItem, charsmax(szItem), "%s %L", szItem, id, "DGM_MENU_LEVEL", eAWP[LEVEL])
			}
			
			if(eAWP[XP] && iXP < eAWP[XP])
				format(szItem, charsmax(szItem), "%s %L", szItem, id, "DGM_MENU_XP", eAWP[XP])
		}
		
		if(eAWP[OWNER] != ADMIN_ALL && !(iFlags & eAWP[OWNER]))
			format(szItem, charsmax(szItem), "%s %L", szItem, id, "DGM_MENU_OWNER_ONLY")

		if(eAWP[FLAG] != ADMIN_ALL && !(iFlags & eAWP[FLAG]))
			format(szItem, charsmax(szItem), "%s %L", szItem, id, "DGM_MENU_VIP_ONLY")
			
		if(g_iAWP[id] == i)
			format(szItem, charsmax(szItem), "%s %L", szItem, id, "DGM_MENU_SELECTED")
	...
	{
		formatex(szItem, charsmax(szItem), "%s%L", szTitle, id, "DGM_MENU_TITLE_PAGE")
...
public MenuHandler(id, iMenu, iItem)
{
	if(iItem != MENU_EXIT)
	{
		...
		CC_SendMessage(id, "%L %L", id, "DGM_CHAT_PREFIX", id, "DGM_CHAT_SELECTED", szName)
...
{		
	...
	if(eAWP[OWNER] != ADMIN_ALL && !(get_user_flags(id) & eAWP[OWNER]))
		return false
