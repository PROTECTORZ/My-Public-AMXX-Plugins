...
public plugin_init()
{
	...
	if(!g_iAWPNum)
		set_fail_state("No AK47`s found in the configuration file.")
	
	register_dictionary("AK47Models.txt")
	
	RegisterHam(Ham_Spawn, "player", "OnPlayerSpawn", 1)
	RegisterHam(Ham_Item_Deploy, "weapon_ak47", "OnSelectAWP", 1)
...
public ShowMenu(id)
{
	...
	formatex(szTitle, charsmax(szTitle), "%L", id, "AKM_MENU_TITLE")
		...
		if(g_bRankSystem)
		{
			if(eAWP[LEVEL] && iLevel < eAWP[LEVEL])
			{
				if(eAWP[SHOW_RANK])
				{
					...
					format(szItem, charsmax(szItem), "%s %L", szItem, id, "AKM_MENU_RANK", szRank)
				}
				else
					format(szItem, charsmax(szItem), "%s %L", szItem, id, "AKM_MENU_LEVEL", eAWP[LEVEL])
			}
			
			if(eAWP[XP] && iXP < eAWP[XP])
				format(szItem, charsmax(szItem), "%s %L", szItem, id, "AKM_MENU_XP", eAWP[XP])
		}

		if(eAWP[OWNER] != ADMIN_ALL && !(iFlags & eAWP[OWNER]))
			format(szItem, charsmax(szItem), "%s %L", szItem, id, "AKM_MENU_OWNER_ONLY")
		
		if(eAWP[FLAG] != ADMIN_ALL && !(iFlags & eAWP[FLAG]))
			format(szItem, charsmax(szItem), "%s %L", szItem, id, "AKM_MENU_VIP_ONLY")
			
		if(g_iAWP[id] == i)
			format(szItem, charsmax(szItem), "%s %L", szItem, id, "AKM_MENU_SELECTED")
	...
	{
		formatex(szItem, charsmax(szItem), "%s%L", szTitle, id, "AKM_MENU_TITLE_PAGE")
...
public MenuHandler(id, iMenu, iItem)
{
	if(iItem != MENU_EXIT)
	{
		...
		CC_SendMessage(id, "%L %L", id, "AKM_CHAT_PREFIX", id, "AKM_CHAT_SELECTED", szName)
