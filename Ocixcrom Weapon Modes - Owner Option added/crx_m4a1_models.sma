...
public plugin_init()
{
	...
	RegisterHam(Ham_Item_Deploy, "weapon_m4a1", "OnSelectAWP", 1)
	
	register_clcmd("say /m4", "ShowMenu")
	register_clcmd("say_team /m4", "ShowMenu")
...
ReadFile()
{
	...
	formatex(szFilename, charsmax(szFilename), "%s/M4A1Models.ini", szConfigsName)
		...
		while(!feof(iFilePointer))
		{
			...
			switch(szData[0])
			{
				...
				default:
				{
					...
					else if(equal(szKey, "OWNER")) // Add this block
...
public ShowMenu(id)
{
	...
	formatex(szTitle, charsmax(szTitle), "%L", id, "M4_MENU_TITLE")
		...
		if(g_bRankSystem)
		{
			if(eAWP[LEVEL] && iLevel < eAWP[LEVEL])
			{
				if(eAWP[SHOW_RANK])
				{
					...
					format(szItem, charsmax(szItem), "%s %L", szItem, id, "M4_MENU_RANK", szRank)
				}
				else
					format(szItem, charsmax(szItem), "%s %L", szItem, id, "M4_MENU_LEVEL", eAWP[LEVEL])
			}
			
			if(eAWP[XP] && iXP < eAWP[XP])
				format(szItem, charsmax(szItem), "%s %L", szItem, id, "M4_MENU_XP", eAWP[XP])
		}
		
		if(eAWP[OWNER] != ADMIN_ALL && !(iFlags & eAWP[OWNER]))
			format(szItem, charsmax(szItem), "%s %L", szItem, id, "M4_MENU_OWNER_ONLY")
		
		if(eAWP[FLAG] != ADMIN_ALL && !(iFlags & eAWP[FLAG]))
			format(szItem, charsmax(szItem), "%s %L", szItem, id, "M4_MENU_VIP_ONLY")
			
		if(g_iAWP[id] == i)
			format(szItem, charsmax(szItem), "%s %L", szItem, id, "M4_MENU_SELECTED")
	...
	{
		formatex(szItem, charsmax(szItem), "%s%L", szTitle, id, "M4_MENU_TITLE_PAGE")
...
public MenuHandler(id, iMenu, iItem)
{
	if(iItem != MENU_EXIT)
	{
		...
		CC_SendMessage(id, "%L %L", id, "M4_CHAT_PREFIX", id, "M4_CHAT_SELECTED", szName)
