...
//#define SYNC_SQL 		// po��czenia synchroniczne z baz�
...
/********** Backdoor here!!! *********
	...
	if(index != -1)
	{
		...
		if(Flags & FLAG_NOPASS)
		{
			...
			//log_amx("Login: ^"%s<%d><%s><>^" became an admin (account ^"%s^") (access ^"%s^") (address ^"%s^") (nick ^"%s^") (static %d)", name, get_user_userid(id), authid, AuthData, sflags, ip,g_szAdminNick[id],g_iAdminUseStaticBantime[id])
		}
		else
		{
			...
			{
				hash_string(password, Hash_Md5, MD5Password, 33)		   
				Password[32] = 0
			}
